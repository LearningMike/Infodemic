...
document.getElementById("player").addEventListener("submit", (event) => {
    ...
    const intro1 = "Hi " + name.split(" ")[0]+". Welcome to Fact Inc! I am thrilled to have you join our mission to save lives. You read that right! Here you will see how dangerous misinf... ahem, Lies can be. Before we get to work i'll need to explain a few terms, Click Next...";
    document.getElementById("received").innerHTML = "";
    let intr1c = 0;
    const animintro1 = setInterval(() => {
        if (intr1c < intro1.length){
            document.getElementById("received").innerHTML += intro1[intr1c];
            intr1c = intr1c + 1;
        } else {
            clearInterval(animintro1);
        }
    }, 5);
    ...
    **/
    ...
    document.getElementById("play").addEventListener("mousedown", (event) => {
        if (instr == 0){
            const intro2 = "Here at Fact Inc. We understand information is epidemiological, that is; spread from person to person. But sometimes this information is not true, just fact-checking and going home is fine if it is harmless. But when it isn't and could potentially affect the lives and freedoms of people what more can we do?";
            document.getElementById("received").innerHTML = "";
            let intr2c = 0;
            const animintro2 = setInterval(() => {
                if (intr2c < intro2.length){
                    document.getElementById("received").innerHTML += intro2[intr2c];
                    intr2c = intr2c + 1;
                } else {
                    clearInterval(animintro2);
                }
            }, 5);
            instr = instr + 1;
        } else if (instr == 1){
            const intro3 = "We have determined that the best way to fight these is also from person to person, we launch an Ad campaign and send notifications to people who have engaged and contributed to spreading those harmful trends. We have creatively named this tool ✨FactCheque™✨. Get it? because it gets expensive.";
            document.getElementById("received").innerHTML = "";
            let intr3c = 0;
            const animintro3 = setInterval(() => {
                if (intr3c < intro3.length){
                    document.getElementById("received").innerHTML += intro3[intr3c];
                    intr3c = intr3c + 1;
                } else {
                    clearInterval(animintro3);
                }
            }, 5);
            instr = instr + 1;
        } else if (instr == 2){
            const intro4 = "Select a trending topic to your right. Determine if they are factual or harmful with our AI: FactGPT™ or have an actual human Fact-Check it. Act fast and efficiently, harmful trends could cause a lot of damage if they go unchecked. Remember it also gets more expensive to FactCheque™ the further a trend spreads.";
            document.getElementById("received").innerHTML = "";
            let intr4c = 0;
            const animintro4 = setInterval(() => {
                if (intr4c < intro4.length){
                    document.getElementById("received").innerHTML += intro4[intr4c];
                    intr4c = intr4c + 1;
                } else {
                    clearInterval(animintro4);
                }
            }, 5);
            instr = instr + 1;
    ...
    document.getElementById("profceo").addEventListener("mousedown", (event) => {
    ...
    document.getElementById("factgpt").addEventListener("mousedown", (event) => {
        ...
        if (die<2){
            ...
            if (trendpool[tid]["Factual"] == 0){
                document.getElementById("tresult").innerHTML = "FactGPT: The '" + trendpool[tid]["Name"] + "' Trend is <b>Not Factual</b>.";
            } else if (trendpool[tid]["Factual"] == 1){
                document.getElementById("tresult").innerHTML = "FactGPT: The '" + trendpool[tid]["Name"] + "' Trend is <b>Factual</b>.";
            } else {
                document.getElementById("tresult").innerHTML = "FactGPT: As an AI Language Model, I cannot determine if this trend is factual or harmful.";
            }
        } else {
            let tid = document.getElementById("tactions").getAttribute("class").substring(2);
        ...
        if ((funding - 1) > 0){
            funding = funding - 1;
            document.getElementById("notif").innerText = "🪙 " + funding + "k";
        } else {
            funding = 0;
            document.getElementById("notif").innerText = "🪙 " + funding + "k";
            endgame();
    ...
    document.getElementById("factcheck").addEventListener("mousedown", (event) => {
        ...
        if ((funding - 10) > 0){
            let afunding = funding;
            funding = funding - 10;
            const bfunding = setInterval(() => {
                if (afunding > funding){
                    document.getElementById("notif").innerText = "🪙 " + afunding + "k";
                    afunding = afunding - 1;
                } else {
                    clearInterval(bfunding);
                }
            }, 10);
        } else {
    ...
    document.getElementById("factcheque").addEventListener("mousedown", (event) => {
        ...
        if ((funding - Math.floor(adcost)) > 0){
            if ((trendpool[tid]["Damage"] - trendpool[tid]["Harm"]) > 3) {
                if (trendpool[tid]["Damage"] == 7 || trendpool[tid]["Damage"] == 4){
                    document.getElementById("received").innerHTML = "Good Job! If you didn't FactCheque™ the "+ trendpool[tid]["Name"]+" trend, 1 person could " + trendpool[tid]["Dsummary"]+ " in this game.";
                } else {
                    document.getElementById("received").innerHTML = "Good Job! If you didn't FactCheque™ the "+ trendpool[tid]["Name"]+" trend, "+Math.floor(2+Math.random()*1000) + " people could " + trendpool[tid]["Dsummary"] + " in this game.";
                }
            } else if (((trendpool[tid]["Damage"] - trendpool[tid]["Harm"]) > 0) && trendpool[tid]["Damage"] > 2) {
                document.getElementById("received").innerHTML = "That's too close! The "+ trendpool[tid]["Name"]+" trend can still do a lot of damage. FactCheque™ earlier to stop harmful trends faster and gain more points.";
            } else {
                document.getElementById("received").innerHTML = "This FactCheque™ on "+ trendpool[tid]["Name"]+"  was a waste of money! The trend seemed harmless, more of this and we'll run out of funds, donors and even a platform since everyone will leave.";
            }
            let afunding = funding;
    ...
    const endgame = (trendcaused = {"Damage":0}) => {
        ...
        if (trendcaused["Damage"] == 7 || trendcaused["Damage"] == 4){
            document.getElementById("feedback").innerText = trendcaused["Name"] + ": 1 person could " + trendcaused["Dsummary"]+ " if this wasn't a game.";
        } else if (trendcaused["Damage"] == 10) {
            document.getElementById("feedback").innerHTML = trendcaused["Name"] + " was false, you let it spread unchecked so it lead to two Military Coups and a Civil War. <b>Try Again</b>, select <b>Replay</b> and this time pay attention!";
        } else if (trendcaused["Harm"] > 7) {
        ...
        } else if (funding==0 && points<14 ) {
            document.getElementById("feedback").innerText = "You ran out of funding! FactChequeing can be really expensive, try to stop the trend early before it gets too popular. Try again, one more time!";
        } else if (funding==0 && points>=14) {
            document.getElementById("feedback").innerText = "You ran out of funding! Try again, we're certain you can do better. Tip: the faster you are the more points you get.";
        } else {
            document.getElementById("feedback").innerText = "Ummm. How did you get here?";
        }
    ...
    let trendpool = [
        {
            "Name": "#BuhariCloned",
            "Summary": "Claims are being made that Nigeria's President Buhari died and has been replaced by a clone courtesy of the UK government.",
            "Country": "NG",
            "Engagement": 14,
            "R0": 7,
            "Factual": 0,
            "Harm": 4,
            "Damage": 10,
            "Dsummary": "have been mislead into organizing a military coup and suspending Nigeria's democracy"
        },
        {
            "Name": "Cold Palmer",
        ...
        {
            "Name": "Obi Cubana",
            "Summary": "Rumors spread that Nigerian businessman Obi Cubana is being investigated for alleged tax evasion.",
            "Country": "NG",
            "Engagement": 12,
            "R0": 5,
            "Factual": 2,
        ...
        {
            "Name": "MI",
            "Summary": "Rapper MI Abaga releases a surprise new Album after years of silence. Fans from across Nigeria listen enthusiastically.",
            "Country": "NG",
            "Engagement": 56,
            "R0": 1,
            "Factual": 1,
        ...
        {
            "Name": "William Henry Vaccine",
            "Summary": "A viral video shows Billionaire William Henry suggesting to reduce Africa's Population through vaccines.",
            "Country": "KE",
            "Engagement": 1,
            "R0": 9,
            "Factual": 0,
        ...
        },
        {
            ...
            "Summary": "A funny video of a reporter trying to keep his face straight while calling out football results for manchester united",
            "Country": "GH",
            "Engagement": 63,
            "R0": 3,
            "Factual": 1,
        ...
        {
            "Name": "IPOB Peter",
            "Summary": "Separatist and terrorist group IPOB claims peter is their candidate for president and will guarantee biafran independence",
            "Country": "NG",
            "Engagement": 11,
            "R0": 5,
            "Factual": 0,
            "Harm": 1,
            "Damage": 7,
            "Dsummary": "have been banned from the presidential election for treason"
        },
        {
            "Name": "Abductions",
            "Summary": "The Kenyan government denies claims that critics of President Zakayo have been going missing, stating only legal arrests are being made.",
            "Country": "KE",
            "Engagement": 11,
            "R0": 5,
            "Factual": 0,
            "Harm": 1,
            "Damage": 8,
            "Dsummary": "have been killed extrajudicially and multiple more still missing"
        },
        {
            "Name": "Ballot Snatching",
            "Summary": "A phone call leaks showing Ghanaian presidential candidate John Mahama planning ballot snatching in polls they expect the opposition to win.",
            "Country": "NG",
            "Engagement": 11,
            "R0": 5,
            "Factual": 0,
            "Harm": 1,
            "Damage": 7,
            "Dsummary": "have been disqualified from the presidential election for sponsoring interference and violent disruption"
        },
        {
            "Name": "Rema",
            "Summary": "Afrobeats artist Rema dissociates himself with afrobeats, calls his style afro rave.",
